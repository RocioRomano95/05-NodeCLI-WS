...
const selectorTypeMatcher = function (selector = "tag") {
  ...
  if (selector.indexOf(".") != -1) {
    return "tag.class"; // selector.includes('.') o split
  }
  return "tag";
};
...
const matchFunctionMaker = function (selector) {
  ...
  let matcher;
  if (selectorType === "id") {
    return function matcher(sampleDivElement) {
      if ("#" + sampleDivElement.id === selector) {//#${sampleDivElement.id === selector}
        return true;
      }
      return false;
    };
    ...
    return function matcher(sampleDivEl) {
      ...
      return sampleDivEl.className.split(" ").includes(selector);
    };
